...
class TestBackward:
    ...
    def test_backward_cross_attention(self):
        ...
        torch.testing.assert_close(
            q.grad, q_ref.grad.permute(0, 2, 1, 3).squeeze(0), rtol=5e-2, atol=5e-2
        )
        torch.testing.assert_close(
            k.grad, k_ref.grad.permute(0, 2, 1, 3).squeeze(0), rtol=5e-2, atol=5e-2
        )
        torch.testing.assert_close(
            v.grad, v_ref.grad.permute(0, 2, 1, 3).squeeze(0), rtol=5e-2, atol=5e-2
        )
    ...
    def test_gradcheck(self):
        ...
        q = torch.randn(
            seq_len, num_heads, head_dim, device="cuda", dtype=torch.float64, requires_grad=True
        )
        k = torch.randn(
            seq_len, num_kv_heads, head_dim, device="cuda", dtype=torch.float64, requires_grad=True
        )
        v = torch.randn(
            seq_len, num_kv_heads, head_dim, device="cuda", dtype=torch.float64, requires_grad=True
        )
